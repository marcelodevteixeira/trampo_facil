...
const ServiceCard: React.FC<ServiceCardProps> = ({ service }) => {
  ...
  const colorClass = CATEGORY_COLORS[service.category] || "bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300";

  return (
    <Link to={`/service/${service.id}`} className="block">
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-all active:scale-[0.99] mb-3">
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-gray-900 dark:text-white line-clamp-1 text-base">{service.title}</h3>
              <p className="text-xs text-gray-500 dark:text-gray-400 font-medium">{service.provider_name}</p>
            </div>
          </div>
        </div>
