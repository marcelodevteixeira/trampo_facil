...
import { fetchServices, createService, supabase, signInWithEmail, signOut, getUserProfile, signUpWithEmail, signInWithGoogle } from './services/supabase';
import { CATEGORY_ICONS, CATEGORY_COLORS, CATEGORY_IMAGES } from './constants';
import { LogOut, Phone, MessageCircle, MapPin, Search, Loader2, ArrowLeft, ChevronDown, ChevronRight, SlidersHorizontal, Grid, Hammer, Home as HomeIcon, Moon, Sun, GraduationCap, Play, Clock, Mail, CheckCircle2, XCircle, Wifi } from 'lucide-react';
...
const Login: React.FC<{ setAuth: any }> = ({ setAuth }) => {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
        if (isRegistering) {
            ...
            // Supabase auth state change will be caught by the useEffect in App component, 
            // but for mock/direct handling we set it here too if needed, though App's listener is better.
            if (!supabase) {
                // If mock
                setAuth({ user: user as any, loading: false, session: session as any });
                navigate('/');
            }
        }
    } catch (error: any) {
      ...
      return (
          <div className="min-h-screen bg-gradient-to-br from-purple-800 to-purple-600 flex flex-col items-center justify-center p-6 text-center text-white relative overflow-hidden">
              ...
              <div className="z-10 w-full mb-12 space-y-4 max-w-sm">
                  <button 
                    onClick={() => { setShowInput(true); setIsRegistering(false); }}
                  ...
                  <button 
                    onClick={() => { setShowInput(true); setIsRegistering(true); }}
                    className="w-full bg-transparent border-2 border-white/40 text-white font-bold py-4 rounded-xl active:scale-[0.98] transition-all hover:bg-white/10 hover:border-white/60"
                  >
                      Criar Conta
                  </button>

                  {/* Connection Status Indicator */}
                  <div className="flex items-center justify-center gap-2 mt-4 text-xs font-medium text-white/60 bg-black/20 py-2 rounded-full backdrop-blur-sm">
                    {connectionStatus === 'checking' && <><Loader2 className="w-3 h-3 animate-spin" /> Verificando conexão...</>}
                    {connectionStatus === 'connected' && <><CheckCircle2 className="w-3 h-3 text-green-400" /> Sistema Online</>}
                    {connectionStatus === 'error' && <><XCircle className="w-3 h-3 text-red-400" /> Sistema Offline (Verifique SQL)</>}
                  </div>
              </div>
          </div>
      );
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900 flex flex-col items-center justify-center p-6 text-center relative overflow-hidden transition-colors duration-200">
       ...
       <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white mb-2">{isRegistering ? 'Criar Conta' : 'Bem-vindo'}</h1>
       <p className="text-gray-500 dark:text-gray-400 mb-8 max-w-xs font-medium">
           {isRegistering ? 'Preencha os dados abaixo.' : 'Insira seus dados para continuar.'}
       </p>

       <form onSubmit={handleEmailAuth} className="w-full max-w-sm space-y-4 relative z-10 mx-auto">
          {isRegistering && (
              <>
                <input type="text" placeholder="Nome Completo" value={name} onChange={e => setName(e.target.value)} className="w-full p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-primary transition-all" required />
                <input type="tel" placeholder="Telefone / WhatsApp" value={phone} onChange={e => setPhone(e.target.value)} className="w-full p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-primary transition-all" required />
              </>
          )}
          <div className="space-y-4">
            <input type="email" placeholder="E-mail" value={email} onChange={e => setEmail(e.target.value)} className="w-full p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-primary transition-all" required />
            <input type="password" placeholder="Senha" value={password} onChange={e => setPassword(e.target.value)} className="w-full p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-primary transition-all" required />
          </div>
          
          <button disabled={loading} className="w-full bg-primary text-white font-bold py-4 rounded-xl shadow-xl shadow-primary/30 active:scale-95 transition-all hover:bg-purple-700 flex items-center justify-center gap-2">
             {loading ? <Loader2 className="animate-spin" /> : (isRegistering ? 'Cadastrar' : <><Mail className="w-5 h-5" /> Entrar com E-mail</>)}
          </button>
       </form>
       ...
       <button 
        ...
        className="w-full max-w-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-white font-bold py-4 rounded-xl shadow-sm active:scale-95 transition-all hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center justify-center gap-3"
       >
           {/* Google Icon SVG */}
       ...
       <button onClick={() => setIsRegistering(!isRegistering)} className="mt-8 text-primary font-bold text-sm hover:underline">
    ...
    return (
        <Layout theme={theme} toggleTheme={toggleTheme}>
            <div className="flex flex-col items-center py-10">
                <div className="w-24 h-24 bg-primary text-white rounded-full flex items-center justify-center text-3xl font-bold mb-4 shadow-lg shadow-primary/30 border-4 border-white dark:border-gray-800 transition-colors overflow-hidden">
                    {user?.avatar_url ? (
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        // --- AUTH LISTENER ---
        // This listens for actual Supabase auth changes (Google login redirects, etc.)
        const { data: { subscription } } = supabase ? supabase.auth.onAuthStateChange(async (event, session) => {
            if (session?.user) {
                ...
                setAuth({ user: null, loading: false, session: null });
            }
        }) : { data: { subscription: { unsubscribe: () => {} } } };

        // Fallback for Mock Mode or initial load check
        if (!supabase) {
